...
from modules import backend as bk, login_win
from tkinter import messagebox as mb


class SuperAdmin():
    def __init__(self, win, list, user_details):
        self.win = win
        self.img_list = list
    ...
    def __sa_gui(self):
        ...
        # Label(self.frame1, text="Student Management System (super admin)", font=(
        #     40)).place(relx=0.5, rely=0.1, anchor=CENTER)
        Button(self.frame1, text="Menu", font=("Verdana", 13), bd=1,
               height=1, command=self.place_label_menu).place(x=20, y=120)
        Button(self.frame1, text="Logout", font=("Verdana", 13), bd=1,
               height=1, command=self.logout).place(x=130, y=120)
        Button(self.frame1, text="Home", bd=1, font=("Verdana", 13),
               command=self.home_icons).place(x=240, y=120)

        self.lbl = Label(height=15, width=13)
        Button(self.lbl, text="Add New\nAdmin",  width=9,
               command=self.new_admin).place(x=0, y=1)
        Button(self.lbl, text="Update Your\nPassword",  width=9,
               command=self.update_password_frame).place(x=0, y=63)
        Button(self.lbl, text=" Manage\nAdmin",  width=9,
               command=self.manage_admin).place(x=0, y=125)
        ...
        self.info_lbl = Label(
            self.frame1, text="A Student Management System (SMS) is an all-encompassing platform that simplifies the management of student data, \n academic records, and administrative tasks for educational institutions. It provides a unified interface for \n tracking academic progress, managing enrollment, and facilitating communication  between students, parents, \n and faculty. By automating administrative processes, it enhances operational efficiency and supports data-driven decision-making", font="lucida 13")
        self.info_lbl.place(relx=0.5, rely=0.5, anchor=CENTER)
    ...
    def update_password_frame(self):
        ...
        Label(self.frame2, text="UPDATE USERNAME & PASSWORD", font=(
            "lucida", 20, "bold", "underline")).place(x=300, y=10)
        Label(self.frame2, text=f"ID : {self.user_details[0]}", font=(
            "lucida", 16)).place(x=375, y=80)
        Label(self.frame2, text="Username : ", font=(
            "lucida", 16)).place(x=300, y=120)
        Label(self.frame2, text="Enter New Password : ", font=(
            "lucida", 16)).place(x=200, y=160)
        Label(self.frame2, text="Confirm Password : ", font=(
            "lucida", 16)).place(x=222, y=200)
        self.uname = StringVar(value=self.user_details[1])
        self.password = StringVar()
        self.confirm_password = StringVar()
        self.uname_txt = Entry(self.frame2, font=(
            "Times New Roman", 16), background="white", width=20, bd=2, textvariable=self.uname)
        self.uname_txt.place(x=420, y=120)
        self.pass_txt = Entry(self.frame2, font=("Times New Roman", 16), background="white",
                              show="•", width=20, bd=2, textvariable=self.password)
        self.pass_txt.place(x=420, y=160)
        self.confirm_pass_txt = Entry(self.frame2, textvariable=self.confirm_password, font=(
            "Times New Roman", 16), background="white", show="•", width=20, bd=2)
        self.confirm_pass_txt.place(x=420, y=200)
        self.count = IntVar()
        Button(self.frame2, text="Update", font=("Verdana", 16), bd=1,
               command=self.update).place(x=350, y=290)
        self.show_pass = Checkbutton(self.frame2, text="Show Password", variable=self.count,
                                     font=("Calibri", 14, "bold"), command=self.show_password).place(x=380, y=230)
    ...
    def new_admin(self):
        ...
        self.name_txt = Entry(self.frame3, textvariable=self.name, font=(
            "Times New Roman", 16), background="white", width=20, bd=2)
        ...
        self.age_box = Spinbox(self.frame3, textvariable=self.age, from_=18, to=70, state="readonly",
                               bg="white", width=3, font=("Times New Roman", 16), bd=2)
        ...
        self.address_txt = Text(self.frame3, bd=2, bg="white", font=(
        ...
        self.apass_txt = Entry(self.frame3, font=(
            "Times New Roman", 16), textvariable=self.apass, background="white", width=20, bd=2)
        self.apass_txt.place(x=350, y=410)
        Button(self.frame3, text="ADD", font=("Verdana", 16),  bd=1,
               width=9, height=1, command=self.validate).place(x=320, y=470)
        Button(self.frame3, text="Reset", font=("Verdana", 16),  bd=1,
               width=9, height=1, command=self.reset).place(x=480, y=470)
    ...
    def manage_admin(self):
        ...
        self.address_txt = Text(self.frame4, bd=2, bg="white", font=(
            "Times New Roman", 14), height=4, width=37)
        ...
        Button(self.frame4, text="UPDATE", font=("Verdana", 16), bd=1,
               width=9, height=1, command=self.update_madmin).place(x=0, y=470)
        Button(self.frame4, text="DELETE", font=("Verdana", 16), bd=1,
               width=9, height=1, command=self.delete_admin).place(x=160, y=470)
        ...
        self.sc = Scrollbar(
            self.frame4, command=self.table.yview, bd=2)
